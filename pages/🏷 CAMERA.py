...
def save_image_to_gcs_and_mysql(image, bucket_name, blob_name, target_size=(192, 256)):
    # 이미지를 PIL Image로 변환
    ...
    client = storage.Client.from_service_account_json('./formidable-pact-405301-3f0dc19a7e71.json')
...
# 파일 업로드 기능을 추가합니다.
uploaded_file = st.file_uploader("또는 파일 업로드(모바일은 꼭!! 업로드!)", type=["jpg", "jpeg"])
...
if uploaded_file is not None or cam is not None:
    if uploaded_file is not None:
        ...
        with st.spinner('제출 중입니다'):
            image_url = save_image_to_gcs_and_mysql(uploaded_file, bucket_name, blob_name, target_size=target_size)
